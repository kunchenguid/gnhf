...
- `acp.ts`: handles `acp:<target>` specs through the bundled `acpx` runtime and registry. It keeps a persistent per-run session keyed by run ID under `.gnhf/runs/<runId>/acp-sessions`, embeds the output schema in the prompt, parses the last completed output message first with rightmost-object extraction fallback, records ACP lifecycle events in `gnhf.log`, and reports per-iteration token deltas from cumulative ACP usage updates with prompt/output estimates when needed. Path and arg overrides are native-agent-only and are not forwarded to ACP targets.

...
- `stream-utils.ts`: shared JSONL parsing, `AbortSignal` wiring, and child-process spawn/lifecycle helpers. Per-iteration CLI agents must spawn through `spawnAgentProcess`, the only place that keeps argv intact across a Windows `.cmd`/`.bat` shim. When touching agent streaming, start here.

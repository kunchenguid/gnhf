...
```bash
gnhf \
  --agent <claude|codex|gemini|rovodev|opencode|copilot|pi|acp:<target>> \
  --max-iterations <n> \
...
pgrep -fl 'gnhf|claude|codex|gemini|copilot|opencode|rovodev' || true
```
...
- `gemini`: explicit Gemini CLI request or general-purpose instruction-following.

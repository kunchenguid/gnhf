...
program
  ...
  .option(
    "--agent <agent>",
    "Agent to use (claude, codex, rovodev, opencode, gemini, copilot, junie, jules, or kilo)",
  )
  .option(
    "--max-iterations <n>",
  ...
  .action(
    async (
      ...
      if (
        ...
        console.error(
          `Unknown agent: ${options.agent}. Use "claude", "codex", "rovodev", "opencode", "gemini", "copilot", "junie", "jules", or "kilo".`,
        );
        process.exit(1);
      }

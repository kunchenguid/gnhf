...
| ACP target         | `--agent acp:<target>` | Install and authenticate the target supported by the bundled `acpx` registry, such as `acp:gemini`. | `gnhf` runs the target through ACP with a persistent per-run session under `.gnhf/runs/<runId>/acp-sessions`; `agentPathOverride` and `agentArgsOverride` do not apply. |

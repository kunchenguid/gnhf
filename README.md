...
- **Agent-agnostic**: works with Claude Code, Codex, Rovo Dev, OpenCode, GitHub Copilot CLI, Cursor Agent CLI, Pi, or ACP targets out of the box
...
| Cursor Agent CLI   | `--agent cursor`                  | Install Cursor's `cursor-agent` CLI and sign in first (requires an active Cursor subscription).                                                                                     | `gnhf` invokes `cursor-agent -p --output-format stream-json --force --trust` in non-interactive mode, appends the final output schema to the prompt, and parses the terminal `result.result` text as JSON when available, falling back to assistant text for older streams. Per-iteration token usage is read from the terminal `result` event's `usage` field when available; older builds without that field keep live heuristic estimates. |

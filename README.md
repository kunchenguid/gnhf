...
| `--max-rate-limit-wait <duration>` | Abort after this much total usage-limit wait (`30m`, `2h`, or `0`)                                 | 24h safety cap         |
...
- For `codex`, `claude`, `copilot`, `cursor`, and `grok`, `gnhf` adds its usual non-interactive permission default only when you do not provide your own permission or execution-mode flag. If you set one explicitly, `gnhf` treats that as user-managed and does not add its default on top.
...
| Grok Build         | `--agent grok`                    | Install xAI's `grok` CLI and sign in first (`grok login`), or set `XAI_API_KEY`.                                                                                                    | `gnhf` invokes `grok -p` directly in `streaming-json` mode with `--json-schema`, reads the final output and exact token usage from grok's `end` event, and defaults to `--always-approve` unless you set `--always-approve`, `--yolo`, `--dangerously-skip-permissions`, or `--permission-mode`. It does not pass `--trust`, so grok skips auto-loading project `AGENTS.md`, skills, hooks, and repo-local MCP servers; adding `--trust` via `agentArgsOverride.grok` opts in but permanently records the folder (and each worktree path) in grok's trust store. After grok emits its `end` event, `gnhf` shuts down any lingering grok process tree after a short grace period. |

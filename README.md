...
| Cursor Agent CLI   | `--agent cursor`                  | Install Cursor's `cursor-agent` CLI and sign in first (requires an active Cursor subscription).                                                                                     | `gnhf` invokes `cursor-agent -p --output-format stream-json --force --trust` in non-interactive mode, appends the final output schema to the prompt, and parses the last assistant message as JSON. Per-iteration token usage is read from the terminal `result` event's `usage` field when the installed `cursor-agent` exposes it (older builds without that field report zero).                                                              |

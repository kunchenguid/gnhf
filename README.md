...
- **Failure handling** - gnhf rolls back failures and checkpoints interruptions. A failed iteration produced wrong work, so it is rolled back with `git reset --hard` - except commit failures, which preserve uncommitted work for repair; agent-reported failures proceed to the next iteration immediately, retryable hard agent errors use exponential backoff, and permanent agent errors such as Claude low credit balance abort immediately after rollback and print the run log path. Complete no-op iterations are reported as failures and count toward the consecutive-failure abort limit. If the run exits with a pending commit failure, the exit summary warns that uncommitted changes were left for repair.
- **Runtime caps** - `--max-iterations` stops before the next iteration begins, `--max-tokens` aborts mid-iteration once reported usage reaches the cap, and `--stop-when` ends the loop after an iteration whose agent output reports the natural-language condition is met unless a commit failure needs repair first; resumed runs reuse the saved stop condition unless you pass a new value, or `--stop-when ""` to clear it. Hitting the token cap is an interruption, not a failure: the in-flight work is committed as a clearly labeled, unverified checkpoint commit (`gnhf <n>: checkpoint (max tokens reached)`, or `chore: checkpoint (max tokens reached)` under the `conventional` commit-message preset) instead of being destroyed, along with any leftover non-gitignored files the agent produced. A checkpoint may contain broken mid-tool-call state - the next resumed iteration is told via `notes.md` to inspect and repair it before building on it, so do not merge a checkpoint tip without resuming first. If the checkpoint commit itself fails (for example strict pre-commit hooks), the work is preserved uncommitted for repair, same as any commit failure. The exit summary flags a run that ended on a checkpoint so the unverified tip is not a surprise. Pass `--token-buffer <n>` with `--max-tokens` to stop between iterations once usage is within `n` tokens of the cap, letting the current turn finish and commit normally - size it to roughly one turn's worth of tokens; the hard cap remains as backstop. Pending commit-failure repair work is always preserved, and in the interactive TUI the final state remains visible until you press Ctrl+C to exit
...
| `--max-tokens <n>`       | Abort after `n` total input+output tokens, checkpointing in-flight work                            | unlimited              |
| `--token-buffer <n>`     | With `--max-tokens`: stop between iterations once usage is within `n` tokens of the cap            | none                   |

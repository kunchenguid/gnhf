...
| ACP target         | `--agent acp:<target>` | Install and authenticate the target supported by the bundled `acpx` registry, such as `acp:gemini`. | `gnhf` runs the target through ACP with a persistent per-run session under `.gnhf/runs/<runId>/acp-sessions`; token usage and `--max-tokens` use ACP `used` deltas when available, estimate prompt/output tokens otherwise, and `agentPathOverride` and `agentArgsOverride` do not apply. |
